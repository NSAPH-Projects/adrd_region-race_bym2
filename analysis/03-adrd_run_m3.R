...
mkdir_p(paste0('results/models/m3/model_run_', tstamp))
model_name <- 'model'
stan_file <- 'analysis/m3_interact.stan'
random_seed <- get_random_seed(paste0('results/models/m3/model_run_', tstamp,
                                      '/seed.rds'))
...
adrd_ratios_df %<>%
  left_join(adrd_county_df %>%
              dplyr::select(county, med_house_income, pm25) %>% distinct(),
            by = "county")
...
write_rds(adrd_stan_list,
          paste0('results/models/m3/model_run_', tstamp, 
                 '/adrd_stan_list.rds'))

## copy stan file into model_run ----
new_stan_file <- paste0('results/models/m3/model_run_', tstamp, '/m_', tstamp, '.stan')
file.copy(stan_file, 
          new_stan_file)
...
fit <- stan(
  ...
  refresh = n_iter / 100,
  sample_file = paste0('results/models/m3/model_run_', tstamp,
                       '/sample_file')
)

## Save fit objects ----
write_rds(fit, 
          paste0('results/models/m3/model_run_', 
                 tstamp, 
                 '/stanfit_object.rds'))


## Remove the compiled stan model ----
file.remove(paste0('results/models/m3/model_run_', tstamp, '/m_', tstamp, '.rds'))
