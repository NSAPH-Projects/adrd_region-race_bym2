...
data {
    ...
    int<lower = 0> pop[m];      // population_years
    ...
    real pm25[m];              // mean PM2.5 over study period
...
parameters {
  ...
  vector[6] beta;                 // Coeffs on principal component(s)
    ...
    for (i in 1:m) {
        if(pop[i] != 0){
          ...
          real log_mu = log_offset[i] + 
                    ...
                    // two coefficients for income
                    beta[1]   * income[i] +                    
                    beta[2]   * income[i] * d2_idx[i] + 
                    // two coefficients for PM2.5
                    beta[3]   * pm25[i] +                    
                    beta[4]   * pm25[i] * d2_idx[i] + 
                    // two coefficients for interaction
                    beta[5]   * income[i] * pm25[i] +
                    beta[6]   * income[i] * pm25[i] * d2_idx[i] +
                    //
