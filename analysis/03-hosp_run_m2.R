...
path_mod <- "results/models/hosp/m2/"
...
#   as.data.frame() %>%
#   filter(row %in% hosp_ratios_df$c_idx & col %in% hosp_ratios_df$c_idx) %>%
#   as.matrix()
# 
...
# income <- (hosp_ratios_df$med_house_income -
#              mean(hosp_ratios_df$med_house_income, na.rm = TRUE)) / 10000

# Mean center (change name here and in model to HPI)
income <- (hosp_ratios_df$house_price_income_ratio -
             mean(hosp_ratios_df$house_price_income_ratio, na.rm = TRUE))
