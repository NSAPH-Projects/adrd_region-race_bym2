...
if __name__ == "__main__":
    ...
    parser.add_argument('--outcomes_prefix', 
                        type=str, 
                        default='data/symlinks/scratch/outcomes_adrd') # written in script 100
    parser.add_argument("--output_format", 
                        default = "parquet", 
                        choices=["parquet", "feather", "csv"]
                       )           
    parser.add_argument("--output_prefix", 
                    default = "data/symlinks/scratch/nom_adrd_dual"
                   )
    args = parser.parse_args()


    logging.basicConfig(filename=f"logs/get_adrd_nom_dual_{args.year}.out", level=logging.INFO)
