...
# python data_processing/06_hosp_rates_sensitivity.py --admissions_county_csv data/symlinks/scratch/adrd_county_sensitivity.csv --output_file data/symlinks/scratch/adrd_rates_sensitivity.csv
# python data_processing/06_hosp_rates_sensitivity.py --admissions_county_csv data/symlinks/scratch/nonadrd_county_sensitivity.csv --output_file data/symlinks/scratch/nonadrd_rates_sensitivity.csv

import pandas as pd
import argparse
import logging

logging.basicConfig(filename='logs/adm_rates_dual.out', level=logging.INFO)
...
def main(args):
    ...
    ## merge with enrollee and adrd/hosp counts
    ...
    dual_ = sorted(bene_county_df.dual.unique())
    ...
    adm_rates_df = adm_rates_df.merge(pd.DataFrame({'dual':dual_}), how = 'cross')
    # add state
    ...
    logging.info("## there are county-years-race-sex-age_grp-duals with zero enrollees ----")
    ...
    logging.info("## total number of adrd/hosp admissions in adm_rates_df ----")
    logging.info(adm_rates_df.n_admissions.sum())

    logging.info("## percentage of county-race-sex-age_grp-dual combinations with missing enrollees (all years) ----")
    logging.info(adm_rates_df.n_enrollees.isnull().mean())

    logging.info("## percentage of county-race-sex-age_grp combinations with missing adrd/hosp hospitalizations (all years) ----")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--admissions_county_csv', 
                        type=str,
                        default='data/symlinks/scratch/nonadrd_county_sensitivity.csv') # edit name here
    parser.add_argument('--output_file',
                        type=str,
                        default='data/symlinks/scratch/nonadrd_rates_sensitivity.csv') # edit name here
